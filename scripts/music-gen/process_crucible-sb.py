"""Post-process crucible-sb raw stems → processed wav+mp3.

Same invariants as flagship-sb: trim to 32s, peak-normalize to -12 dBFS,
50 ms edge fades. Sox reverb per layer, tuned for the timbre:
  - ambient: long deep hall (60/65/85) — brooding bed should bloom
  - melodic: tight punchy plate (28/70/65) — keeps brass-stab marcato
  - layer3:  moderate hall (45/65/75) — horn wants air; timpani gets weight
"""
...
def main():
    ...
    rev = RAW / "crucible-sb-ambient-reverb.wav"
    sox_reverb(RAW / "crucible-sb-ambient.wav", rev, 60, 65, 85, pre_delay_ms=30)
    y, sr = load_stereo(rev)
    y = trim_or_pad(y, sr, LOOP_S)
    y = fade_edges(y, sr)
    y = normalize_peak(y, -12.0)
    results.append({"name": "crucible-sb-ambient",
                    **write_wav_and_mp3(y, sr, "crucible-sb-ambient")})

    rev = RAW / "crucible-sb-melodic-reverb.wav"
    sox_reverb(RAW / "crucible-sb-melodic.wav", rev, 28, 70, 65, pre_delay_ms=18)
    y, sr = load_stereo(rev)
    y = trim_or_pad(y, sr, LOOP_S)
    y = fade_edges(y, sr)
    y = normalize_peak(y, -12.0)
    results.append({"name": "crucible-sb-melodic",
                    **write_wav_and_mp3(y, sr, "crucible-sb-melodic")})

    rev = RAW / "crucible-sb-layer3-reverb.wav"
    sox_reverb(RAW / "crucible-sb-layer3.wav", rev, 45, 65, 75, pre_delay_ms=25)
    y, sr = load_stereo(rev)
    y = trim_or_pad(y, sr, LOOP_S)
    y = fade_edges(y, sr)
    y = normalize_peak(y, -12.0)
    results.append({"name": "crucible-sb-layer3",
                    **write_wav_and_mp3(y, sr, "crucible-sb-layer3")})

    summary = PROC / "_crucible-sb_summary.json"
