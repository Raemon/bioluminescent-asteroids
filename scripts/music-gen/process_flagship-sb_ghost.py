"""Post-process for flagship-sb-melodic + flagship-sb-layer3 (ghost-duet take).

Standard invariants: trim to 32 s, 50 ms edge fades, peak-normalize to
-12 dBFS, write wav + mp3 to processed/. Sox reverb tuned per timbre:
  - melodic (ghost lead): moderate plate, modest pre-delay — air without
    washing out the vibrato
  - layer3 (glass bells): big hall — strikes should bloom and recede
"""
...
def main():
    ...
    sox_reverb(RAW / "flagship-sb-melodic.wav", rev, 50, 60, 80, pre_delay_ms=20)
    ...
    sox_reverb(RAW / "flagship-sb-layer3.wav", rev, 68, 55, 95, pre_delay_ms=30)
    y, sr = load_stereo(rev)
    y = trim_or_pad(y, sr, LOOP_S)
    y = fade_edges(y, sr)
    y = normalize_peak(y, -12.0)
    ...
    summary = PROC / "_flagship_ghost_summary.json"
