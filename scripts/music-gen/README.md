...
| `flagship-sb` | fully procedural (numpy) all three stems — `build_flagship-sb.py` (ambient) + `build_flagship-sb_ghost.py` (melodic + layer 3) | pulsing 16th-note arp (ambient) + theremin-like ghost lead with detuned unison, delayed vibrato, portamento and dotted-quarter echo (melodic, G4–G5) + sparse inharmonic glass-bell strikes with ping-pong echo placed in the lead's rests (layer 3, G5–E6); haunting duet over the pulse | 1.0·0.5 / 0.5 (per music-config.json) |
