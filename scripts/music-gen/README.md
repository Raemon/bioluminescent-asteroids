...
| `synthwave-el` | ElevenLabs end-to-end synthwave (ambient+melodic) + numpy plucked detuned-saw echo cascade via `build_synthwave-el_layer3_v2.py` (layer 3) | Juno-pad + soft lead; layer 3 = sparse C5-C6 pluck motif whose dotted-eighth ping-pong delay cascades through the upper register both other stems leave empty (87% of their energy is below 500 Hz) | 0.22 / 0.38 |
| `flagship-sb` | procedural numpy arp (ambient, `build_flagship-sb.py`) + ElevenLabs solo nylon guitar (melodic) + ElevenLabs solo violin (layer 3). Best 32s windows via `find_loop_flagship_reroll.py`, processed by `process_flagship_reroll.py`; guitar HPF'd 120 Hz, violin HPF'd 300 Hz. | pulsing 16th-note arp (ambient) + lush slow lyrical nylon-string guitar with hall reverb (melodic) + singing legato solo violin in the upper register (layer 3); cinematic and beautiful, EL acoustic voices over the procedural pulse | 1.0·0.25 / 0.30 (per music-config.json) |
...
| `outerwilds-el` | ElevenLabs Music all three stems — 64s pieces, best 32s window via `find_loop_outerwilds-el.py`. Melodic guitar onsets snapped to 8th-note grid via `quantize_to_beat.py` (piecewise rubberband). Ambient HPF'd at 200 Hz, layer3 HPF'd at 500 Hz to keep bass kit clear. Layer3 is **not** quantized — bursts read as musical at the EL output's natural timing. | Outer-Wilds folk: distant drone pad + fingerpicked acoustic guitar (G mixolydian — V-over-I suspension) + sparse plucked acoustic-guitar countermelody (D-centered upper-register single-note plucks, sits above the fingerpicking); haunting/gentle | 0.235 / 0.30 |
| `crucible-sb` | numpy sine pad + C1 sub-pulse (ambient); FluidSynth GM 61 brass section (melodic); FluidSynth GM 60 French horn + GM 47 timpani (layer 3). **Boss-fight only — kept OUT of `HALO_MUSIC_POOL`; force-picked by `syncHaloAmbient` when `isBossWave(game.wave)`.** | climactic C-minor boss theme — brooding pedal + heartbeat sub (ambient) / stabbing marcato brass riff climbing minor pentatonic (melodic) / heroic French horn call-and-response + sparse C2/G2 timpani downbeat strikes (layer 3). 24x climax crossfade is suppressed on the boss wave so this track plays for the entire engagement. | 0.30 / 0.32 |
| `knell-sb` | numpy sine pad + C1 lub-dub heartbeat + G5 shimmer (ambient); FluidSynth GM 48 string-ensemble staccato ostinato + GM 44 tremolo swells (melodic); FluidSynth GM 52 ghost choir + GM 14 tubular bells (layer 3) — `build_knell-sb.py`. **Alternate boss-fight candidate — kept OUT of `HALO_MUSIC_POOL`; auditionable on /music, swap `BOSS_MUSIC_VARIATION` to hear in-game.** | haunting C-minor death-knell boss theme — brooding pedal + double-pulse heartbeat (ambient) / relentless eighth-note string ostinato with tremolo builds (melodic) / Dies-irae-shaded ghost-choir descent + tolling bell on phrase downbeats (layer 3). B phrase stops the ostinato and lifts to Cmaj7 before resolving minor. | 0.30·0.32 / 0.32 |
| `cathedral-hymn-el` | ElevenLabs all three stems — 64s pieces, best 32s window via `find_loop_haunting-el.py`. No HPF; the chant layer is intentionally bass-register. **Haunting post-boss pool only — kept OUT of `HALO_MUSIC_POOL`, lives in `HAUNTING_MUSIC_POOL` selected on internal waves 12–20 (display levels 11–19).** | stone-cathedral hymn: slow bowed-string pad in long reverb + distant felt-piano single tones + sotto-voce low monastic male chant on 'ooo/aaa' vowels (layer 3); sacred and slightly otherworldly | 0.27 / 0.30 |
| `lost-transmission-el` | ElevenLabs all three stems — 64s pieces, mid-heavy so gains held low. Layer3 is non-instrumental texture (breathy whispers + radio crackle). **Haunting post-boss pool only.** | AM-radio ghost transmission: warm analog pad through tape-hiss + wow/flutter + bandlimiting (ambient) / frail wavering musical-saw lead (melodic) / subliminal whispered breaths and faint short-wave static (layer 3); frail, cold, lonely | 0.22 / 0.28 |
| `underwater-requiem-el` | ElevenLabs all three stems — 64s pieces. Layer3 HPF'd at 500 Hz so the celesta sits above the bass kit. **Haunting post-boss pool only.** | submerged requiem: heavy-lowpass orchestral string pad + watery shimmer reverb (ambient) / faint sustained glass-harmonica tones (melodic) / sparse upper-register celesta countermelody (layer 3); mournful and beautiful | 0.25 / 0.32 |
| `spectral-toll-sb` | ElevenLabs ambient + melodic (64s pieces, best 32s window via `find_loop_flagship_reroll.py`, processed by `process_flagship_reroll.py`); procedural numpy glass bells (layer 3, reused from `build_flagship-sb_ghost.py`, parked as `glassbells-sb-layer3`). **Haunting post-boss pool only.** | spectral toll: deep C/G open-fifth drone, bowed bass + low choir (ambient) / mournful slow legato solo cello (melodic) / sparse inharmonic glass-bell strikes with dotted-quarter ping-pong echo placed in the cello's rests (layer 3, G5–E6); sorrowful and beautiful | 0.27 / 0.45 |
