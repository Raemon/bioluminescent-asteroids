...
// ship's own forecast path — dots mark where the ship will be at successive beats if it keeps
...
export const renderShipTrajectoryPreview = (
  ...
  for (let k = 1; k <= SHIP_TRAJECTORY_BEAT_COUNT; k++) {
    ...
    // dots further out fade so the eye reads "near future is reliable, far future is a hint".
