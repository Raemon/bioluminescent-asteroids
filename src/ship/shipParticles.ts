...
export const emitThrust = (ship: Ship, particles: ParticleSystem, t: number) => {
  ...
  particles.emit({
    pos: tail,
    vel: add(mul(spread, 180 + rng() * 120), mul(ship.vel, 0.3)),
    life: 0.4 + rng() * 0.2, maxLife: 0.6,
    size: 1.4 * flicker, hue: 190 + rng() * 40,
    shrink: 1, drag: 1.8,
  });
};
...
const emitReverseCore = (ship: Ship, particles: ParticleSystem, muzzle: { x: number; y: number }, flicker: number) => {
  for (let i = 0; i < 2; i++) {
    ...
    particles.emit({
      pos: muzzle,
      vel: add(mul(spread, 220 + rng() * 140), mul(ship.vel, 0.3)),
      life: 0.18 + rng() * 0.12, maxLife: 0.3,
      size: 1.6 * flicker, hue: 195 + rng() * 25,
      shrink: 1, drag: 2.4,
    });
  }
};
...
const emitReverseHalo = (ship: Ship, particles: ParticleSystem, muzzle: { x: number; y: number }, flicker: number) => {
  const haloJitter = (rng() - 0.5) * 0.8;
  const haloSpread = fromAngle(ship.heading + haloJitter, 1);
  particles.emit({
    pos: muzzle,
    vel: add(mul(haloSpread, 90 + rng() * 80), mul(ship.vel, 0.25)),
    life: 0.35 + rng() * 0.2, maxLife: 0.55,
    size: 2.2 * flicker, hue: 210 + rng() * 20,
    shrink: 1, drag: 1.6,
  });
};
...
const emitReverseSpark = (ship: Ship, particles: ParticleSystem, muzzle: { x: number; y: number }) => {
  if (rng() >= 0.5) return;
  const sparkJitter = (rng() - 0.5) * 0.5;
  const sparkDir = fromAngle(ship.heading + sparkJitter, 1);
  particles.emit({
    pos: muzzle,
    vel: add(mul(sparkDir, 320 + rng() * 160), mul(ship.vel, 0.2)),
    life: 0.12 + rng() * 0.08, maxLife: 0.2,
    size: 1.0, hue: 50 + rng() * 30,
    shrink: 1, drag: 3.0,
  });
};
...
export const emitSideThrust = (ship: Ship, particles: ParticleSystem, t: number, side: "port" | "starboard") => {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    particles.emit({
      pos: muzzle,
      vel: add(mul(spread, 200 + rng() * 120), mul(ship.vel, 0.25)),
      life: 0.2 + rng() * 0.14, maxLife: 0.34,
      size: 1.5 * flicker, hue: 28 + rng() * 22,
      shrink: 1, drag: 2.2,
    });
  }
  const haloJitter = (rng() - 0.5) * 0.9;
  const haloSpread = fromAngle(ventAngle + haloJitter, 1);
  particles.emit({
    pos: muzzle,
    vel: add(mul(haloSpread, 80 + rng() * 70), mul(ship.vel, 0.2)),
    life: 0.32 + rng() * 0.18, maxLife: 0.5,
    size: 2.0 * flicker, hue: 18 + rng() * 20,
    shrink: 1, drag: 1.6,
  });
};
