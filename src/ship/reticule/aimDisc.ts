...
// Why: when the disc is directly over an object (vs. just near a lock dot), add a hard flicker so
...
// Why: shadow-blur halo turns the flash into a glow that bleeds outside the dashed ring.
...
// Why: stricter "directly on" check — the reticule's centre is inside the target's body. Used for
...
// Why: triangle wave in [0, DEPTH] driven by beatTime — reads as a clear flicker, not a slow pulse.
